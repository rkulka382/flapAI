...
def eval_genomes(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        while not showGameOverScreen:
            # Get the inputs from the game state
            inputs = game.get_state()
            # Feed the inputs to the network and get the output
            output = net.activate(inputs)
            # Perform the action based on the output
            game.jump() if output[0] > 0.5 else game.do_nothing()
            # Update the game state
            game.update()
            # Increment the fitness score by the reward
...
def main():
    ...
    while True:
        ...
        # **Create a config object from the config file**
        config = neat.Config(neat.DefaultGenome, neat.DefaultReproduction, neat.DefaultSpeciesSet, neat.DefaultStagnation, config_file)
        # **Create a population object from the config object**
...
config_file = 'aiConfig.txt'
# Run the run function and get the best genome
best_genome = run(config_file)
